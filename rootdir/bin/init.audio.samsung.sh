...
/system/bin/setprop "vendor.audio.a2dp.hasrun" "false"
...
do
	...
	# A2DP call state
	a2dpstate=$(/system/bin/getprop vendor.audio.a2dp.call)
	...
	epc=$(/system/bin/getprop vendor.audio.earpiece.voip)
	spk=$(/system/bin/getprop vendor.audio.speaker.voip)
	...
	# A2DP call sim1
	if [[ $callstatus == "0" ]] && [[ "$(/system/bin/getprop vendor.audio.a2dp.hasrun)" == "false" ]] && [[ "$(/system/bin/getprop vendor.audio.a2dp.pickup)" == "true" ]]; then
		$log -t SamsungAudio "In phone call"
		if [[ $a2dpstate == "true" ]]; then
			$log -t SamsungAudio "In A2DP phone call, setting A2DP ctls, sim1"
			/system/bin/setprop "vendor.audio.a2dp.hasrun" "true"
			$log -t SamsungAudio "Prop set, moving on"
			...
			$tinymix "SLIM_7_RX_Voice Mixer VoiceMMode1" 1
			$tinymix "VoiceMMode1_Tx Mixer SLIM_7_TX_MMode1" 1
			$log -t SamsungAudio "Done!!!!!"
		fi
	fi

	# A2DP call sim2
	if [[ $callstatus == "1" ]] && [[ "$(/system/bin/getprop vendor.audio.a2dp.hasrun)" == "false" ]] && [[ "$(/system/bin/getprop vendor.audio.a2dp.pickup)" == "true" ]]; then
		$log -t SamsungAudio "In phone call"
		if [[ $a2dpstate == "true" ]]; then
			$log -t SamsungAudio "In A2DP phone call, setting A2DP ctls, sim2"
			/system/bin/setprop "vendor.audio.a2dp.hasrun" "true"
			$log -t SamsungAudio "Prop set, moving on"
			$tinymix "QUIN_MI2S_RX_Voice Mixer VoiceMMode1" 0
			$tinymix "QUIN_MI2S_RX_Voice Mixer VoiceMMode2" 0
	...
	# A2DP call reset
	if [[ $callstatus == "-1" ]] && [[ "$(/system/bin/getprop vendor.audio.a2dp.hasrun)" == "true" ]]; then
	/system/bin/setprop "vendor.audio.a2dp.hasrun" "false"
	$log -t SamsungAudio "Prop reset, moving on"
	fi
