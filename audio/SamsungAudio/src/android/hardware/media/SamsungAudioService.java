    ...
    public void onCreate() {
        ...
        mRunnable = new Runnable() {
            public void run() {
                ...
                communicationState = mAudioManager.getCommunicationDevice().getType();

                if (mAudioManager.getMode() == AudioManager.MODE_IN_COMMUNICATION) {

                    if (communicationState == AudioDeviceInfo.TYPE_BUILTIN_EARPIECE) {
                        Log.e(TAG, "Earpiece");
                        SystemProperties.set("vendor.audio.earpiece.voip", "true");
                        SystemProperties.set("vendor.audio.speaker.voip", "false");
                    }

                    if (communicationState == AudioDeviceInfo.TYPE_BUILTIN_SPEAKER) {
                        Log.e(TAG, "Speaker");
